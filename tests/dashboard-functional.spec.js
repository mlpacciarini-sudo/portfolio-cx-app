...
test.describe("Portfolio de Proyectos CX - pruebas funcionales", () => {
  ...
  test("3. Ver ficha abre el proyecto seleccionado", async ({ page }) => {
    ...
    await expect(
      page.getByText("Resumen ejecutivo", { exact: true })
    ).toBeVisible();

    await expect(
      page.getByText("Entregables e hitos", { exact: true })
    ).toBeVisible();
  });
  ...
  test("5. Los entregables pueden desplegarse o contraerse", async ({ page }) => {
    ...
    const deliverableSection = page.getByText(
      "Entregables e hitos",
      { exact: true }
    );
    ...
    const toggleButtons = section.locator("button");
